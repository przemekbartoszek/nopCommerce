...
namespace Nop.Plugin.Api.Controllers
{
    public class CustomersController : BaseApiController
    {
        ...
        public async Task<IActionResult> GetCustomerById(int id, string fields = "")
        {
            ...
            var customer = await _customerApiService.GetCustomerById(id);
        ...
        {
            var allCustomersCount = await _customerApiService.GetCustomersCount();
        ...
        public async Task<IActionResult> UpdateCustomer([ModelBinder(typeof(JsonModelBinder<CustomerDto>))] Delta<CustomerDto> customerDelta)
        {
            ...
            if (customerDelta.Dto.RoleIds.Count > 0)
            {
                await AddValidRoles(customerDelta, currentCustomer);
            }
            ...
            await InsertFirstAndLastNameGenericAttributes(customerDelta.Dto.FirstName, customerDelta.Dto.LastName, currentCustomer);
            ...
            if (!string.IsNullOrWhiteSpace(customerDelta.Dto.Password))
            {
                await AddPassword(customerDelta.Dto.Password, currentCustomer);
            }
            ...
            await PopulateAddressCountryNames(updatedCustomer);

            // Set the fist and last name separately because they are not part of the customer entity, but are saved in the generic attributes.
            var firstNameGenericAttribute = (await _genericAttributeService.GetAttributesForEntityAsync(currentCustomer.Id, nameof(Customer)))
            ...
            var lastNameGenericAttribute = (await _genericAttributeService.GetAttributesForEntityAsync(currentCustomer.Id, nameof(Customer)))
            ...
            var languageIdGenericAttribute = (await _genericAttributeService.GetAttributesForEntityAsync(currentCustomer.Id, nameof(Customer)))
        ...
        private async Task InsertFirstAndLastNameGenericAttributes(string firstName, string lastName, Customer newCustomer)
        {
        ...
        private async Task AddValidRoles(Delta<CustomerDto> customerDelta, Customer currentCustomer)
        {
        ...
        private async Task PopulateAddressCountryNames(CustomerDto newCustomerDto)
        {
            foreach (var address in newCustomerDto.Addresses)
            {
                await SetCountryName(address);
            }

            if (newCustomerDto.BillingAddress != null)
            {
                await SetCountryName(newCustomerDto.BillingAddress);
            }

            if (newCustomerDto.ShippingAddress != null)
            {
                await SetCountryName(newCustomerDto.ShippingAddress);
            }
        }

        private async Task SetCountryName(AddressDto address)
        {
