    ...
    public class CustomerApiService : ICustomerApiService
    {
        ...
        public async Task<int> GetCustomersCount()
        {
            ...
            return _customerRepository.Table.Count(customer => !customer.Deleted
                                      && (customer.RegisteredInStoreId == 0 || customer.RegisteredInStoreId == storeId));
        }
        ...
        public async Task<CustomerDto> GetCustomerById(int id, bool showDeleted = false)
        {
            if (id == 0)
                return null;
        ...
        private async Task<int> GetDefaultStoreLangaugeId()
        {
            // Get the default language id for the current store.
            var defaultLanguageId = (await _storeContext.GetCurrentStoreAsync()).DefaultLanguageId;
            ...
            if (defaultLanguageId == 0)
            {
                ...
                var storeLanguages = allLanguages.Where(l =>
                    AsyncHelper.RunSync(() => _storeMappingService.AuthorizeAsync(l, storeId))).ToList();
        ...
        private async Task SetNewsletterSubscriptionStatus(IEnumerable<CustomerDto> customerDtos)
        {
            ...
            var allNewsletterCustomerEmail = await GetAllNewsletterCustomersEmails();

            foreach (var customerDto in customerDtos)
            {
                await SetNewsletterSubscriptionStatus(customerDto, allNewsletterCustomerEmail);
            }
        }

        private async Task SetNewsletterSubscriptionStatus(BaseCustomerDto customerDto, IEnumerable<string> allNewsletterCustomerEmail = null)
        {
            ...
            {
                allNewsletterCustomerEmail = await GetAllNewsletterCustomersEmails();
            }
        ...
        private async Task<IEnumerable<string>> GetAllNewsletterCustomersEmails()
        {
            var storeId = (await _storeContext.GetCurrentStoreAsync()).Id;
            return await _cacheManager.GetAsync(Configurations.NEWSLETTER_SUBSCRIBERS_KEY, () =>
            {
                IEnumerable<string> subscriberEmails = (from nls in _subscriptionRepository.Table
                                                        where nls.StoreId == storeId
                                                              && nls.Active
