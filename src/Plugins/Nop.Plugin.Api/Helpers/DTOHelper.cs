...
namespace Nop.Plugin.Api.Helpers
{
    public class DTOHelper : IDTOHelper
    {
        ...
        public async Task<ProductDto> PrepareProductDTO(Product product)
        {
            var productDto = product.ToDto();

            var productPictures = await _productService.GetProductPicturesByProductIdAsync(product.Id);
            await PrepareProductImages(productPictures, productDto);
            ...
            productDto.AssociatedProductIds = await GetRelatedProductIdsByProductId(product.Id);

            // load product attributes
            var productAttributeMappings = await _productAttributeService
                .GetProductAttributeMappingsByProductIdAsync(product.Id);

            await PrepareProductAttributes(productAttributeMappings, productDto);
            

            // load product specification attributes
            var productSpecificationAttributeMappings = await _specificationAttributeService
        ...
        private async Task<List<int>> GetRelatedProductIdsByProductId(int productId)
        {
            ...
            products = products.Where(p => AsyncHelper.RunSync(() => _aclService.AuthorizeAsync(p)) && AsyncHelper.RunSync(() => _storeMappingService.AuthorizeAsync(p))).ToList();
            //availability dates
        ...
        public async Task<ImageMappingDto> PrepareProductPictureDTO(ProductPicture productPicture)
        {
            return await PrepareProductImageDto(productPicture);
        }
        protected async Task<ImageMappingDto> PrepareProductImageDto(ProductPicture productPicture)
        {
        ...
        public async Task<CategoryDto> PrepareCategoryDTO(Category category)
        {
            var categoryDto = category.ToDto();

            var picture = await _pictureService.GetPictureByIdAsync(category.PictureId);
            var imageDto = await PrepareImageDto(picture);
        ...
        public async Task<OrderDto> PrepareOrderDTO(Order order)
        {
            try
            {
                var orderDto = order.ToDto();

                orderDto.OrderItems = (await _orderService.GetOrderItemsAsync(order.Id)).Select(x => AsyncHelper.RunSync(() => PrepareOrderItemDTO(x))).ToList();
                ...
                var customerDto = await _customerApiService.GetCustomerById(order.CustomerId);
        ...
        public async Task<ShoppingCartItemDto> PrepareShoppingCartItemDTO(ShoppingCartItem shoppingCartItem)
        {
            var dto = shoppingCartItem.ToDto();
            dto.ProductDto = await PrepareProductDTO(await _productService.GetProductByIdAsync(shoppingCartItem.ProductId));
        ...
        public async Task<OrderItemDto> PrepareOrderItemDTO(OrderItem orderItem)
        {
            ...
            dto.Product = await PrepareProductDTO(await _productService.GetProductByIdAsync(orderItem.ProductId));
            dto.Attributes = _productAttributeConverter.Parse(orderItem.AttributesXml);
            return dto;
        }

        public async Task<StoreDto> PrepareStoreDTO(Store store)
        {
        ...
        public async Task<LanguageDto> PrepareLanguageDto(Language language)
        {
        ...
        private async Task PrepareProductImages(IEnumerable<ProductPicture> productPictures, ProductDto productDto)
        {
            ...
            {
                var imageDto = await PrepareImageDto(await _pictureService.GetPictureByIdAsync(productPicture.PictureId));
        ...
        protected async Task<ImageDto> PrepareImageDto(Picture picture)
        {
            ImageDto image = null;
        ...
        private async Task PrepareProductAttributes(IEnumerable<ProductAttributeMapping> productAttributeMappings,
            ProductDto productDto)
        {
            ...
            foreach (var productAttributeMapping in productAttributeMappings)
            {
                var productAttributeMappingDto =
                    await PrepareProductAttributeMappingDto(productAttributeMapping);
        ...
        private async Task<ProductAttributeMappingDto> PrepareProductAttributeMappingDto(
        ...
        public async Task<CustomerDto> PrepareCustomerDTO(Customer customer)
        {
        ...
        public async Task<TaxCategoryDto> PrepareTaxCategoryDTO(TaxCategory taxCategory)
        {
        ...
        public async Task<ProductReviewDto> PrepareProductReviewDTO(ProductReview productReview)
        {
