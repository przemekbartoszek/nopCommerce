        ...
        public bool IsCustomerInRole()
        {
            try
            {
                ...
                if (customerIdClaim != null && Guid.TryParse(customerIdClaim.Value, out var customerGuid))
                {
                    ...
                    var customer = AsyncHelper.RunSync(() => customerService.GetCustomerByGuidAsync(customerGuid));

                    if (customer != null)
                    {
                        var customerRoles = AsyncHelper.RunSync(() => customerService.GetCustomerRolesAsync(customer));
